...
$newQuantity = se($_REQUEST, "new_Quantity", 0, false);

if ($user_id > 0 && $newQuantity > 0 && $line_id > 0) {
    $db = getDB();
    $stmt = $db->prepare("UPDATE Cart SET desired_quantity = :desired_quantity where id = :id and :uid");
    try {
        //added user_id to ensure the user can only delete their own items
        $stmt->execute([":id" => $line_id, ":uid" => $user_id, ":desired_quantity" => $newQuantity]);
        $response["status"] = 200;
